...
function parsePnlReport(report, cellIndex) {
  ...
  const overheads = expenseRows
    .filter(r => !WAGE_KEYWORDS.test(r.label))
    .reduce((sum, r) => sum + r.value, 0);
  return { revenue, cogs, wagesSuper, overheads, _debugTotals: totals, _debugExpenseRows: expenseRows };
}

/* Read-only diagnostic: shows exactly which P&L lines were found and how each
   was classified (wage vs overhead vs excluded), without changing any data.
   Login-protected; not linked from the dashboard UI. */
async function apiDebugAccounting(env, url) {
  ...
  try {
    ...
    const reportUrl = 'https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss?fromDate=' + from + '&toDate=' + to;
    const report = await h.fetchJson(reportUrl, { headers: { 'Xero-Tenant-Id': tenantId, 'Accept': 'application/json' } });
    if (url.searchParams.get('raw') === '1') {
      /* Dump just the section/row shape (titles + row types + labels + amount
         column) - enough to see the real structure without the full report. */
      const shape = (rows, depth) => (rows || []).map(r => ({
        depth,
        type: r.RowType,
        title: r.Title || null,
        label: r.Cells && r.Cells[0] && r.Cells[0].Value || null,
        amountCol1: r.Cells && r.Cells[1] && r.Cells[1].Value || null,
        children: r.Rows ? shape(r.Rows, depth + 1) : undefined
      }));
      const rows = (report && report.Reports && report.Reports[0] && report.Reports[0].Rows) || [];
      return json({ shape: shape(rows, 0) });
    }
    const parsed = parsePnlReport(report, 1);
    return json({
      revenue: parsed.revenue, cogs: parsed.cogs, wagesSuper: parsed.wagesSuper, overheads: parsed.overheads,
      totalsFound: parsed._debugTotals,
      expenseLines: parsed._debugExpenseRows.map(r => ({ label: r.label, value: r.value, classified: WAGE_KEYWORDS.test(r.label) ? 'wage' : 'overhead' }))
    });
  } catch (err) {
  ...
  async fetch(request, env) {
    try {
      ...
      if (path === '/api/debug/accounting' && request.method === 'GET') {
        if (!loggedIn) return json({ error: 'auth' }, 401);
        return apiDebugAccounting(env, url);
      }
