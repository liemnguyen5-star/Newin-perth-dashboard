...
function walkPnlRows(rows, cellIndex, totals, expenseRows, inExcludeZone) {
  ...
  for (const r of rows) {
    ...
    /* Income and Cost of Sales lines are never part of Overheads/Wage%, however
       the org's report groups sections (e.g. Wages/Super as their own "Labour"
       section outside "Operating Expenses") - so track an exclude-zone for
       Income/COGS specifically, rather than an include-zone for Operating
       Expenses, which varies too much between orgs to rely on. */
    ...
    if (r.RowType === 'SummaryRow' && r.Cells && r.Cells[cellIndex]) {
      ...
      if (!isNaN(v)) totals[lower] = v;
    }
    if (r.RowType === 'Row' && !nowExcluded && r.Cells && r.Cells[cellIndex]) {
      const v = parseFloat(r.Cells[cellIndex].Value);
