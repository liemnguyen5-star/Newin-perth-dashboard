...
function sumSectionCell(section, cellIndex) {
  if (!section || !section.Rows) return 0;
  let total = 0;
  for (const row of section.Rows) {
    if (row.RowType === 'SummaryRow' && row.Cells && row.Cells[cellIndex]) {
      const v = parseFloat(row.Cells[cellIndex].Value);
      if (!isNaN(v)) return v; /* trust the section's own summary row */
    }
  }
  /* fallback: sum plain Rows if no SummaryRow present */
  for (const row of section.Rows) {
    if (row.RowType === 'Row' && row.Cells && row.Cells[cellIndex]) {
      const v = parseFloat(row.Cells[cellIndex].Value);
      if (!isNaN(v)) total += v;
    }
  }
...
function wageSuperFromOpex(opexSection, cellIndex) {
  if (!opexSection || !opexSection.Rows) return 0;
  let total = 0;
  for (const row of opexSection.Rows) {
    if (row.RowType === 'Row' && row.Cells && row.Cells[0] && WAGE_KEYWORDS.test(row.Cells[0].Value || '')) {
      const v = parseFloat(row.Cells[cellIndex] ? row.Cells[cellIndex].Value : NaN);
      if (!isNaN(v)) total += v;
    }
  }
  return total;
...
function parsePnlReport(report, cellIndex) {
  ...
  let income = null, cos = null, opex = null;
  for (const r of rows) {
    if (r.RowType !== 'Section') continue;
    const title = (r.Title || '').toLowerCase();
    if (title.includes('income') || title.includes('trading')) income = r;
    else if (title.includes('cost of sales') || title.includes('cost of goods')) cos = r;
    else if (title.includes('operating expenses') || title.includes('expenses')) opex = r;
  }
  const revenue = sumSectionCell(income, cellIndex);
  const cogs = sumSectionCell(cos, cellIndex);
  const opexTotal = sumSectionCell(opex, cellIndex);
  const wagesSuper = wageSuperFromOpex(opex, cellIndex);
