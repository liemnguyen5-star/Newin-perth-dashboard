  ...
  accounting: {
    ...
    async status(env, h) {
      ...
      try {
        ...
        const tenant = (conns && conns[0]) || {};
        return {
          connected: true,
          org: tenant.tenantName || '(unknown organisation)',
          sandbox: /demo company/i.test(tenant.tenantName || '')
        };
      } catch (e) {
        return { connected: false, error: { plain: plainError(e.status || 500) } };
      }
    },
    async fetchRange(env, h, q) {
      const parsed = await fetchXeroPnl(env, h, q.from, q.to);
      await h.noteSync();
      return parsed;
    },
    async fetchMonthly(env, h, q) {
      /* Xero's `periods` param is capped at 12: split into <=12-month chunks and stitch. */
      const allMonths = monthRange(q.fromMonth, q.toMonth);
      const chunks = [];
      for (let i = 0; i < allMonths.length; i += 12) chunks.push(allMonths.slice(i, i + 12));
      ...
      for (const chunk of chunks) {
        ...
        const perMonth = await fetchXeroPnlMonthly(env, h, fromDate, toDate, chunk.length);
        out.months.push(...chunk);
        out.revenue.push(...perMonth.revenue);
        out.cogs.push(...perMonth.cogs);
        out.wagesSuper.push(...perMonth.wagesSuper);
        out.overheads.push(...perMonth.overheads);
      }
      await h.noteSync();
      return out;
    }
    ...
    async status(env, h) {
      ...
      try {
        const data = await h.fetchJson('https://connect.squareup.com/v2/locations', {
          headers: { Authorization: 'Bearer ' + token, 'Square-Version': '2026-05-20' }
        }, { auth: false });
        const locs = (data && data.locations) || [];
        const names = locs.map(l => l.name).filter(Boolean).join(', ');
        return { connected: true, org: names || '(no locations found)', sandbox: false };
      } catch (e) {
        return { connected: false, error: { plain: plainError(e.status || 500) } };
      }
    },
    async fetchRange(env, h, q) {
      const count = await squareCompletedCount(env, h, q.from, q.to, q.tz);
      await h.noteSync();
      return { count };
    },
    async fetchMonthly(env, h, q) {
      const months = monthRange(q.fromMonth, q.toMonth);
      const counts = [];
      for (const m of months) counts.push(await squareCompletedCount(env, h, m + '-01', monthEndDate(m), q.tz));
      await h.noteSync();
...
/* ---------------- Xero P&L fetch + parse (accounting adapter helpers) ----
   Reads the Reports[0].Rows[] shape: Section rows carry a Title ("Income" /
   "Cost of Sales" / "Operating Expenses") and nested Rows; SummaryRow gives
   the section total. Wage/super lines are matched by keyword inside
   Operating Expenses and the owner confirms the exact list at
...
/* Recursively walk the report tree collecting every SummaryRow's label -> value,
   plus every plain Row's label -> value (for wage/super keyword detection inside
   Operating Expenses). This is far more reliable than trusting top-level section
   Titles alone: Xero always labels section totals explicitly ("Total Cost of
   Sales", "Total Operating Expenses", etc.) regardless of how sections are
   nested, so matching those exact labels is the same approach a human reading
   the report would use. */
const SUBTOTAL_LABEL = /^(gross profit|net profit|net income|net loss|total)/i;

function walkPnlRows(rows, cellIndex, totals, expenseRows, inExcludeZone) {
  if (!Array.isArray(rows)) return;
  for (const r of rows) {
    const label = (r.Cells && r.Cells[0] && r.Cells[0].Value || '').trim();
    const lower = label.toLowerCase();
    const sectionTitle = (r.Title || '').toLowerCase();
    const nowExcluded = inExcludeZone || (r.RowType === 'Section' &&
      /income|trading|cost of sales|cost of goods/.test(sectionTitle));
    if (r.RowType === 'SummaryRow' && r.Cells && r.Cells[cellIndex]) {
      const v = parseFloat(r.Cells[cellIndex].Value);
      /* First occurrence wins: some Xero report layouts repeat a subtotal
         label in more than one place (e.g. a recap line) - the first, top-down
         one is the section's real total. */
      if (!isNaN(v) && !(lower in totals)) totals[lower] = v;
    }
    /* Some orgs' reports render computed lines like "Gross Profit" / "Net Profit"
       as an ordinary Row rather than a SummaryRow - these are formulas, never
       real cost/income lines, so they must never be summed as an expense. */
    if (r.RowType === 'Row' && !nowExcluded && !SUBTOTAL_LABEL.test(label) && r.Cells && r.Cells[cellIndex]) {
      const v = parseFloat(r.Cells[cellIndex].Value);
      if (!isNaN(v)) expenseRows.push({ label, value: v });
    }
    if (r.Rows) walkPnlRows(r.Rows, cellIndex, totals, expenseRows, nowExcluded);
  }
}

function findTotal(totals, patterns) {
  for (const [label, v] of Object.entries(totals)) {
    for (const p of patterns) if (p.test(label)) return v;
  }
  return 0;
}

function parsePnlReport(report, cellIndex) {
  const rows = (report && report.Reports && report.Reports[0] && report.Reports[0].Rows) || [];
  const totals = {};
  const expenseRows = [];
  walkPnlRows(rows, cellIndex, totals, expenseRows, false);

  const revenue = findTotal(totals, [/^total income$/, /^total revenue$/, /^total trading income$/]);
  const cogs = findTotal(totals, [/^total cost of sales$/, /^total cost of goods sold$/]);
  /* Overheads and Wage%/super are computed by summing every individual expense
     line outside Income/Cost of Sales, split by the wage keyword - not by
     trusting a "Total Operating Expenses" label, since some orgs report wages
     as their own separate section (already excluded from that total) while
     others list them inside Operating Expenses (included in that total). Direct
     summation of leaf lines gives the same correct answer either way. */
  const wagesSuper = expenseRows
    .filter(r => WAGE_KEYWORDS.test(r.label))
    .reduce((sum, r) => sum + r.value, 0);
  const overheads = expenseRows
    .filter(r => !WAGE_KEYWORDS.test(r.label))
    .reduce((sum, r) => sum + r.value, 0);
...
async function xeroTenantId(env, h) {
  const cached = await env.TOKENS.get('tenantId:accounting');
  if (cached) return cached;
  const conns = await h.fetchJson('https://api.xero.com/connections', {});
  const tenant = (conns && conns[0]) || {};
  if (tenant.tenantId) await env.TOKENS.put('tenantId:accounting', tenant.tenantId);
  return tenant.tenantId;
}

async function fetchXeroPnl(env, h, fromDate, toDate) {
  ...
  return parsePnlReport(report, 1); /* single period: label col 0, amount col 1 */
}

async function fetchXeroPnlMonthly(env, h, fromDate, toDate, periods) {
  const tenantId = await xeroTenantId(env, h);
  const url = 'https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss'
    + '?fromDate=' + fromDate + '&toDate=' + toDate
    + '&periods=' + periods + '&timeframe=MONTH';
  const report = await h.fetchJson(url, {
    headers: { 'Xero-Tenant-Id': tenantId, 'Accept': 'application/json' }
  });
  const out = { revenue: [], cogs: [], wagesSuper: [], overheads: [] };
  /* cell 0 = label, cells 1..periods = one amount column per month, oldest first */
  for (let i = 1; i <= periods; i++) {
    const m = parsePnlReport(report, i);
...
/* ---------------- Square: completed transaction count -------------------
   The dashboard's page already computes cur/prev/yoy date ranges in the
   venue's timezone honouring week start and trading-day rollover (see
   README's contract) - by the time q.from/q.to reach here they are already
   the correct calendar dates. This just needs to turn "local midnight to
   local end-of-day" into the precise UTC instants Square's date_time_filter
   expects. -------------------------------------------------------------- */

function tzOffsetMinutes(tz, atUtcDate) {
  try {
    const dtf = new Intl.DateTimeFormat('en-US', { timeZone: tz, timeZoneName: 'shortOffset' });
    const part = dtf.formatToParts(atUtcDate).find(p => p.type === 'timeZoneName');
...
function localToUtcIso(dateStr, timeStr, tz) {
  ...
  const offsetMin = tzOffsetMinutes(tz || 'Australia/Perth', naiveUtc);
  return new Date(naiveUtc.getTime() - offsetMin * 60000).toISOString();
}

async function squareLocationIds(env, h) {
  const cached = await env.TOKENS.get('locationIds:pos');
  if (cached) return JSON.parse(cached);
  const token = env.POS_API_TOKEN;
  const data = await h.fetchJson('https://connect.squareup.com/v2/locations', {
    headers: { Authorization: 'Bearer ' + token, 'Square-Version': '2026-05-20' }
  }, { auth: false });
  const ids = ((data && data.locations) || []).filter(l => l.status !== 'INACTIVE').map(l => l.id);
  if (ids.length) await env.TOKENS.put('locationIds:pos', JSON.stringify(ids));
  return ids;
}

async function squareCompletedCount(env, h, fromDate, toDate, tz) {
  const startAt = localToUtcIso(fromDate, '00:00:00', tz);
  const endAt = localToUtcIso(toDate, '23:59:59', tz);
  const token = env.POS_API_TOKEN;
  const locationIds = await squareLocationIds(env, h);
  let cursor, count = 0;
  do {
    const body = {
      return_entries: true,
    ...
    const data = await h.fetchJson('https://connect.squareup.com/v2/orders/search', {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + token, 'Square-Version': '2026-05-20', 'Content-Type': 'application/json' },
      body: JSON.stringify(body)
    }, { auth: false });
    const entries = (data && (data.order_entries || data.orders)) || [];
...
/* Read-only diagnostic: shows exactly what the Square Orders Search call
   sends and gets back, without changing any data. Login-protected; not
   linked from the dashboard UI. */
async function apiDebugPos(env, url) {
  const from = url.searchParams.get('from');
  const to = url.searchParams.get('to');
  const tz = url.searchParams.get('tz') || 'Australia/Perth';
  if (!from || !to) return json({ error: 'pass ?from=YYYY-MM-DD&to=YYYY-MM-DD' }, 400);
  const token = env.POS_API_TOKEN;
  const h = makeHelpers(env, 'pos');
  const startAt = localToUtcIso(from, '00:00:00', tz);
  const endAt = localToUtcIso(to, '23:59:59', tz);
  try {
    const locationIds = await squareLocationIds(env, h);
    const body = {
      return_entries: true,
    ...
    let parsed = null;
    try { parsed = JSON.parse(text); } catch (e) { /* leave null */ }
    return json({
      hasToken: !!token,
      locationIds,
      requestSent: { startAt, endAt, tz },
      responseStatus: res.status,
      responseBody: parsed || text
    });
  } catch (err) {
    return json({ hasToken: !!token, requestSent: { startAt, endAt, tz }, error: String(err && err.message || err) }, 500);
  }
}
...
export default {
  async fetch(request, env) {
    try {
      ...
      if (path === '/api/debug/pos' && request.method === 'GET') {
        if (!loggedIn) return json({ error: 'auth' }, 401);
        return apiDebugPos(env, url);
      }
      ...
      /* Never show a raw crash: log the detail server-side, tell the owner plainly. */
      console.log('unhandled error: ' + (err && err.stack || err));
      return new Response('Something on the dashboard hit a snag. Your AI can see the detail and will fix it - try again in a moment.', { status: 500 });
    }
  },
